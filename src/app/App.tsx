...
import { Search, ShoppingCart, X, Play, Youtube, Music, ChevronDown, Star } from "lucide-react";
import {
  BarChart,
  Bar,
  XAxis,
                ...
                {i === score.previewPages.length - 1 && score.pages > score.previewPages.length && (
                  <div className="absolute inset-0 bg-gradient-to-t from-[#0d0c14]/80 via-[#0d0c14]/30 to-transparent flex items-center justify-center">
                    <span className="text-xs font-mono text-foreground/70 text-center px-3">
                      +{score.pages - score.previewPages.length} more pages<br />after purchase
                    </span>
                  </div>
                )}
              </div>
  ...
  return (
    <div className="bg-card border border-border rounded-lg p-5">
      ...
      <ResponsiveContainer width="100%" height={220}>
        <BarChart data={GENRE_DATA} layout="vertical" margin={{ left: 0, right: 16, top: 0, bottom: 0 }}>
